...
module StaticAnalysis.Parsing (module Parseable) where
...
import StaticAnalysis.ComplianceChecking
...
instance Parseable Config where
    display (s,q) = "\"" ++ (stripChars "\"" ("(" ++ (display s) ++ ", " ++ (display q) ++ ")")) ++ "\""
...
instance Parseable SyncTransition where
    display (c, e, cc) = (display c) ++ " -> " ++ (display cc) ++ " [label = \"" ++ (display e) ++ "\"];\n"
